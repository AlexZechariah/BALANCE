...
function sessionHeaders(session, mutable = false, extra = {}) {
  return {
    ...extra,
    cookie: session.cookieHeader,
    ...(mutable ? { 'x-csrf-token': session.csrfToken } : {})
  };
}

async function login(email) {
  const result = await fetchJsonResponse('/auth/login', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email, password: 'replace-this-local-only' })
  });
  const session = {
    cookieHeader: cookieHeaderFromResponse(result.response),
    csrfToken: result.body?.csrfToken
  };
  assert(typeof session.csrfToken === 'string' && session.csrfToken.length > 20, `login did not return a CSRF token for ${email}`);
  return { session, user: result.body.user };
}
...
const upload = await fetchJson('/documents', {
  method: 'POST',
  headers: sessionHeaders(consumer.session, true),
  body: form
});
...
const preview = await fetch(`${apiBase}/documents/${documentId}/preview`, {
  headers: sessionHeaders(consumer.session)
});
...
const correction = await fetchJson(`/documents/${documentId}/corrections`, {
  method: 'PATCH',
  headers: sessionHeaders(consumer.session, true, { 'content-type': 'application/json' }),
  body: JSON.stringify({
...
const reviewQueue = await fetchJson('/reviews/queue', {
  headers: sessionHeaders(reviewer.session)
});
...
const reviewClaim = await fetchJson(`/reviews/${claim.review.id}/claim`, {
  method: 'POST',
  headers: sessionHeaders(reviewer.session, true)
});
...
const approval = await fetchJson(`/reviews/${claim.review.id}/approve`, {
  method: 'POST',
  headers: sessionHeaders(admin.session, true, { 'content-type': 'application/json' }),
